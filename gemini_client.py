...
규칙:
1. 동의어/유사어를 반드시 포함하세요 (예: 빵집 → 베이커리, 외식업)
2. 구체적 니즈를 키워드로 변환하세요 (예: 돈이 필요 → 자금지원, 대출)
3. 지역이 언급되면 "소상공인 [지역]" 형태로 포함하세요
4. 최소 3개, 최대 8개의 키워드를 생성하세요
5. 반드시 JSON 배열 형식으로만 응답하세요 (다른 텍스트 없이)

사용자 설명: "{user_description}"

응답 예시: ["베이커리 지원", "외식업 마케팅", "소상공인 홍보", "소상공인 서울"]"""
